<?php
	...
	final class formx_field_checkbox extends formx_field_basic {
		
		public function __construct($field_name = '', $method = '') {
        	
			parent::set_name($field_name)->set_type('checkbox')->set_method($method);
        }
        ...
        final public function set_post_value($selected = '') {
			...
			for($i=0; $i<$total; $i++) {
			    ...
			    $field_post_value[$i]['selected'] 		= (in_array($field_value[$i]['field_value'], $selected) && $selected[0] !== '' ? true : false);
			}
