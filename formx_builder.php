<?php
	...
	/**
	 * A simple html form creator class
	 * 
	 * @author zechim
	 */
	...
	/**
	 ...
	 if ($formx->is_post()) {
		...
		if ($formx->has_error() == false) {
			...
			$formx->get_posted('html_field_name');
		}
	}
	...
	*/	
	...
	class formx_builder {
		...
		public function __construct($form_method = 'post', $form_action = '', $form_id = '', $form_class = '', $form_name = 'formx', $form_enctype = 'multipart/form-data') {

			$form_method 	= strtolower($form_method);
			$form_name 		= $form_name == '' ? 'formx' : $form_name;
			
			if (!preg_match('#^https?://#i', $form_action)) {
					
				$form_action = BASE_URL . ltrim($form_action, '/');
			}
			
			# Allows only post or get for security purpose
			$this->form_method 	= in_array($form_method, array('post', 'get')) ? $form_method : 'post';
			...
			require_once get_formx_field_path() . 'formx_field_basic.php';
		...
		private function add_field($field_type = '', $field_name = '') {
			...
			require_once get_formx_field_path() . 'formx_field_' . $field_type . '.php';
		...
		public function render($formx_view = 'basic') {
			
			return load::view('formx/' . $formx_view, array(
